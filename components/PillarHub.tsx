    ...
    <>
      ...
      <div className="bg-dark-900 border-b border-dark-700/50">
        <div className="max-w-article mx-auto px-4 sm:px-6 py-8 sm:py-12">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="font-display text-4xl sm:text-5xl text-white tracking-tight">{h1}</h1>
          </div>
      ...
      <article className="max-w-article mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        {coreArticles.length > 0 && (
          <div className="mb-10">
            <p className="text-[16px] uppercase tracking-[0.2em] text-light-400 font-medium mb-4">Hovedartikler</p>
            <div className="grid sm:grid-cols-2 gap-3">
              {coreArticles.map((a) => (
                <Link key={a.href} href={a.href}
                  ...
                  <p className="text-lg font-medium text-light-900 group-hover:text-pulse-700 transition-colors">{a.title}</p>
                  <p className="text-[16px] text-light-400 mt-1">{a.desc}</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        {moreArticles.length > 0 && (
          <div>
            <p className="text-[16px] uppercase tracking-[0.2em] text-light-400 font-medium mb-4">Alle artikler</p>
            <div className="border border-light-200 rounded-xl overflow-hidden divide-y divide-light-200">
              {moreArticles.map((a) => (
                <Link key={a.href} href={a.href}
                  ...
                  <span className="text-[17px] text-light-700 group-hover:text-light-900">{a.title}</span>
